...
function draw() {
  ...
  for (let i = COLUMN_BORDER; i < NUMBER_OF_COLUMNS - COLUMN_BORDER; i++) {
...
function drawSquares(squares, column) {
  ...
  for (let j = 0; j < squares.length; j++) {
    ...
    if (isSquareOutsideColumn(squares[j], column)) {
      ...
      if (random() * OUT_DENSITY < abs(DIMENSIONS[1] / 2 - squares[j].middle.y) / (DIMENSIONS[1] / 2))
        continue;
    } else {
      if (random() > IN_DENSITY)
        continue;
    }
    ...
    handDrawRectangleMiddle(
      {
        x: randomGaussian(squares[j].middle.x, SQUARE_POS_SD),
        y: randomGaussian(squares[j].middle.y, SQUARE_POS_SD)
      },
      column.width * 0.4,
      randomGaussian(column.width * squares[j].size - column.width * 0.75, SQUARE_HEIGHT_SD)
    );
  }
...
function getHandDrawnLine(a, b) {
  ...
  for (let i = 0; i < pointsInLine.length; i++) {
    let displacement = SKETCHINESS * noise(i * 0.2);
