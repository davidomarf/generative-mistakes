...
class Heart {
  ...
  updateTimeToNextBeat() {
    ...
    if (this.nextBeatIn-- === 0) {
      ...
      this.nextBeat = abs(ceil(randomGaussian((900 - mouseX) / 10, 1)));
...
class ECG {
  ...
  updateHemoglobin() {
    document.getElementById("hemoglobin-value").innerHTML = randomGaussian(
      14,
      0.1
    ).toFixed(1);
  }
  updateTemperature() {
    document.getElementById("temperature-value").innerHTML = randomGaussian(
      98.6,
      0.1
    ).toFixed(1);
  }
  updateVentilation() {
    document.getElementById(
      "minute-ventilation-value"
    ).innerHTML = randomGaussian(6, 0.5).toFixed(2);
  }

  updateBloodPressure() {
    document.getElementById("pressure-value").innerHTML =
      "" +
      round(randomGaussian(130, 0.1)) +
      "/" +
      round(randomGaussian(90, 0.1));
  }
