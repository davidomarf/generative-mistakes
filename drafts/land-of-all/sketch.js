...
function setup() {
  ...
  // background("#000000");
  drawRandomCircles(7000);
  noLoop();
}

function drawRandomCircles(n, randomColor = false) {
  push();
  noFill();
  strokeWeight(1);
  stroke(0);
  for (let i = 0; i < n; i++) {
    if (randomColor) {
      colorMode(HSL);
      stroke(random(140, 160), 30, 8);
    }
    circle(random(2200), random(2200), random(100, 2000));
  }
  pop();
}
